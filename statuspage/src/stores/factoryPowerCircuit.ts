...
interface FactoryPowerCircuitFilters {
  selectedFactoryTypes: string[]
  selectedSubPowerCircuitIds: number[]
  selectedFactoryStatuses: string[]
}

export const useFactoryPowerCircuitStore = defineStore('factoryPowerCircuit', () => {
  const factories = ref<Factory[]>([])
  const isLoading = ref(false)
  const error = ref<string | null>(null)
  const lastUpdated = ref<Date | null>(null)

  const filters = ref<FactoryPowerCircuitFilters>({
  ...
  const updateFilters = (newFilters: FactoryPowerCircuitFilters) => {
